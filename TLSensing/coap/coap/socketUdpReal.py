...
log = logging.getLogger('Core.management.commands.acquire_values_oneshot')
...
class socketUdpReal(socketUdp.socketUdp):
    ...
    def __init__(self,ipAddress,udpPort,callback):
        ...
        # log
        log.debug('CoAP: creating instance')
    ...
    def run(self):
        while self.goOn:
            try:
                ...
                log.critical("CoAP: socket error: {0}".format(err))
                self.goOn = False
                continue
            else:
                if not raw:
                    log.error("CoAP: no data read from socket, stopping")
                    self.goOn = False
                    continue
                if not self.goOn:
                ...
                log.debug("CoAP: got {2} from {1} at {0}".format(timestamp,source,data))
                ...
                try:
                    ...
                    #import pdb
                    #pdb.set_trace()
                    log.debug("CoAP: SystemError catch: " + str(ex))
