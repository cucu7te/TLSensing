...
class UriQuery(coapOption):
    def __init__(self, argument):
        # initialize parent
        coapOption.__init__(self,d.OPTION_NUM_URIQUERY)

        # store params
        self.argument = argument

    def __repr__(self):
        return 'UriQuery(argument={0})'.format(self.argument)

    def getPayloadBytes(self):
        return [ord(b) for b in self.argument]
...
def parseOption(message,previousOptionNumber):
    ...
    elif optionNumber==d.OPTION_NUM_URIQUERY:
        option = UriQuery(argument=''.join([chr(b) for b in optionValue]))
