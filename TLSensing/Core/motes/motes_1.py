...
#####        request_uri = 'coap://[{0}]{1}{2}'.format(mote_ipv6, resource, query)
        ...
        for res in sensor_list:
            ...
            logger.info(payload)
            logger.info(', '.join(map(str, sensor_data)))
	...
	#c = coap.coap() # create a CoAP instance
        #c.ackTimeout=3
        #c.respTimeout=5
        #mote_ip = 'bbbb::12:4b00:0433:' + str(node_id) # full node address
        #payload = c.GET(('coap://[{0}]' + str(resource)).format(mote_ip)) # CoAP GET call to node
	#		  coap://[bbbb::12:4b00:433:ecdc]/light


        # Query the resource
#####
	#coap_obj.ackTimeout=3
	#coap_obj.respTimeout=5

#####


        # Unpack data from the payload
	#####(sensordata.humidity, sensordata.temperature) = struct.unpack_from('=ff',
        #####    bytearray(payload[sensor_constants.HUMIDITY_LOWER:sensor_constants.TEMPERATURE_UPPER]))
        #####logger.info("*** humidity: " + str(sensordata.humidity))
        #####logger.info("*** temperature: " + str(sensordata.temperature))
        #####(sensordata.light,) = struct.unpack_from('=f',
        #####    bytearray(payload[sensor_constants.LIGHT_LOWER:sensor_constants.LIGHT_UPPER]))
